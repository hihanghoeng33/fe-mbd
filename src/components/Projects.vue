...
const loadProjects = async (page = 1) => {
  ...
  try {
    ...
    console.log(`Loaded ${transformedProjects.length} projects for page ${page}`);
  } catch (error) {
...
const goToPage = async (page) => {
  if (page >= 1 && page <= maxPage.value && page !== currentPage.value && !loading.value) {
  ...
  <div class="gap-y-6">
    ...
    <div class="flex gap-x-16 gap-y-4">
      ...
      <button class="flex items-center gap-x-2 text-gray-600 hover:text-gray-800">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
          <path fill="currentColor" d="M11 18h2c0 .55-.45 1-1 1s-1-.45-1-1zM3 6l2 2v6.5c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V8l2-2V5H3v1zm4.5 8.5L9 13l1.5 1.5L12 13l1.5 1.5L15 13l1.5 1.5L18 13V8.5l-1.5-1.5h-9L6 8.5V13l1.5 1.5z"/>
        </svg>
    ...
    <!-- Pagination Info -->
    <div v-if="totalCount > 0" class="text-sm text-gray-600 mb-4 flex justify-between items-center">
      <div>
        Menampilkan {{ (currentPage - 1) * perPage + 1 }}-{{ Math.min(currentPage * perPage, totalCount) }} dari {{ totalCount }} proyek
        <span v-if="maxPage > 1">(Halaman {{ currentPage }} dari {{ maxPage }})</span>
      </div>
    ...
    <!-- Loading State -->
    <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-y-10">
      <div v-for="i in perPage" :key="`loading-${i}`" class="bg-gray-200 rounded-lg w-96 h-64 animate-pulse"></div>
    </div>
    
    <!-- Daftar Proyek -->
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-y-10">
      <ProjectCardMain
    ...
    <div v-if="maxPage > 1" class="flex justify-center items-center mt-8 gap-2">
      ...
      <div class="flex gap-1">
        ...
        <template v-if="getPageNumbers()[0] > 1">
          ...
          <span v-if="getPageNumbers()[0] > 2" class="px-2 py-2 text-sm text-gray-500">...</span>
        </template>
        ...
        <template v-if="getPageNumbers()[getPageNumbers().length - 1] < maxPage">
          <span v-if="getPageNumbers()[getPageNumbers().length - 1] < maxPage - 1" class="px-2 py-2 text-sm text-gray-500">...</span>
          <button 
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24">
          <path fill="currentColor" d="M5.59 7.41L10.18 12l-4.59 4.59L7 18l6-6l-6-6zM16 6v12h2V6z"/>
        </svg>
      </button>
    </div>
    
    <!-- Page Jump Input (for large datasets) -->
    <div v-if="maxPage > 10" class="flex justify-center items-center mt-4 gap-2 text-sm">
      ...
      <button 
        ...
        :disabled="loading || !jumpToPage || jumpToPage < 1 || jumpToPage > maxPage"
