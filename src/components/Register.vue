<template>
  <div
    class="container flex flex-col items-center justify-center min-h-screen min-w-screen bg-gradient-to-l from-cyan-500 to-blue-500"
  >
    <div
      class="bg-black/70 p-8 rounded-lg shadow-md w-full max-w-sm gap-y-4 backdrop-blur-md text-white flex flex-col"
    >
      ...
      <div
        v-if="errorMessage"
        class="bg-red-500 text-white p-3 rounded mb-4 text-sm"
      >
        {{ errorMessage }}
      </div>
      ...
      <div
        v-if="successMessage"
        class="bg-green-500 text-white p-3 rounded mb-4 text-sm"
      >
        {{ successMessage }}
      </div>
...
const handleRegister = async () => {
  ...
  if (!isValidEmail(formData.value.email)) {
    errorMessage.value =
      "Email must contain @ symbol with a valid user ID before it.";
    loading.value = false;
    return;
  }
  ...
  try {
    const registrationData = {
      ...
      role:
        formData.value.email[0] === "1" &&
        formData.value.email.endsWith("@if.its.ac.id")
          ? "dosen"
          : "mahasiswa",
    };
